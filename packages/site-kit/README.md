...
`NoteHeader.astro` owns everything above a note's body: back navigation, an eyebrow naming the
kind, the heading, the summary, and a row carrying the status, the `badges` slot and the tags —
plus raw/copy actions where the instance publishes a source. `ContentNote.astro` composes it and
adds what a page needs around a header: the `metadata` and `reference` slots and the article
boundary. `TagChips.astro` owns tag markup. An instance passes `tagBase` only when it has a browse
route; without one the same controlled metadata renders without a false link.
...
<ContentNote
  ...
  eyebrow={TYPE_LABELS[entry.data.type]}
...
`eyebrow` is a resolved string, never a kind the frame looks up: the label table belongs where the
kinds are declared and typed against them, so adding a kind is a compile error there rather than an
eyebrow printing a raw type string here. `status` renders as `data-status` for the reason the
licence badge uses `data-policy` — the vocabulary and its colours stay with the instance, and a
value it has not styled still reads. `showHeading` defaults to true; a Markdown body that opens with
its own `<h1>` passes false, and that heading is then what search reads as the title.

Import `NoteHeader.astro` directly for a header outside a note page. The slots are the boundary:
package facts, evidence contracts, source metadata, and domain labels stay with the instance.
Neither component inspects frontmatter or knows any domain kind. They name the roles in
`CONTENT_READER_TOKENS` and `NOTE_HEADER_TOKENS`; consumers can check them with
`contentReaderStyleGaps(css)` and `noteHeaderStyleGaps(css)`.
...
The kit ships the cases its components are meant to handle, as props — every component in the
package, which a test measures against the directory rather than against a list here:
...
The repository's executable [component gallery](../../docs/development/component-gallery.md)
renders the complete `SPECIMENS` array under both a contract-minimum stylesheet and an opinionated
one. Treat that app as the current integration example; an instance owns its gallery route and
presentation because those are part of its theme, while the package owns the cases and their
surface constraints.
...
| Import                                             | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@galaxy-foundry/site-kit`                         | `SiteIdentity`, `ShellLink`, `ResolvedShellLink`, `ResolvedNav`, `resolveNav`, `shellBase`, `shellHref`, `CONTAINER`, `SHELL_TOKENS`, `SHELL_CLASSES`, `shellStyleGaps`, `styleGaps`, `CONTENT_READER_TOKENS`, `contentReaderStyleGaps`, `REFERENCE_TOKENS`, `referenceStyleGaps`, `LICENSE_BADGE_TOKENS`, `licenseBadgeStyleGaps`, `LICENSE_FILE_ROUTE`, `licenseFileHref`, `LicenseFileUse`, `licensesUnderFile`, `LICENSE_FILE_TOKENS`, `licenseFileStyleGaps`, `ResolvedReference`, `PAGEFIND_BODY_ATTR`, `searchIndexGaps`, `SiteShellProps`, `SiteHeaderProps`, `SiteFooterProps`, `ContentNoteProps`, `NoteHeaderProps`, `NOTE_HEADER_TOKENS`, `noteHeaderStyleGaps`, `TagChipsProps`, `ReferenceContractProps`, `LicenseBadgeProps`, `LicenseFileBodyProps` |
| `@galaxy-foundry/site-kit/specimens`               | `Specimen`, `SpecimenGroup`, `SpecimenSurface`, `SPECIMENS`, `CONTENT_NOTE_SPECIMENS`, `NOTE_HEADER_SPECIMENS`, `TAG_CHIPS_SPECIMENS`, `REFERENCE_SPECIMENS`, `LICENSE_BADGE_SPECIMENS`, `LICENSE_FILE_SPECIMENS`, `HEADER_SPECIMENS`, `FOOTER_SPECIMENS`, `SHELL_SPECIMENS`, `SPECIMEN_CONTRACT`, `sharesPage`, `specimenPath`                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `@galaxy-foundry/site-kit/NoteHeader.astro`        | the note frame — everything above the body                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `@galaxy-foundry/site-kit/ContentNote.astro`       | that frame plus the metadata and reference slots and the article boundary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `@galaxy-foundry/site-kit/TagChips.astro`          | controlled tag markup, linked only when the instance supplies a browse route                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
